...
import { AlertDialog, Card, Flex } from '@sesucoin/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const sesucoinper_input = ip_parsed.limit;
    ...
    const sesucoinper_value = Number.parseInt(Number(sesucoinper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (sesucoin per interval):{' '}
                  {mojo_to_sesucoin_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = sesucoin_to_mojo(fee_input.value);
