...
import { AlertDialog } from '@sesucoin/core';
import {
  Typography,
  Button,
  Box,
...
import { sesucoin_to_mojo } from '../../../util/sesucoin';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      sesucoinper_input.value === '' ||
      Number(sesucoinper_input.value) === 0 ||
      !Number(sesucoinper_input.value) ||
      isNaN(Number(sesucoinper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const sesucoinper = sesucoin_to_mojo(sesucoinper_input.value);
    const sesucoinper_value = Number.parseInt(Number(sesucoinper));
    const userpubkey = userpubkey_input.value;
    const amount = sesucoin_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = sesucoin_to_mojo(fee_input.value);
