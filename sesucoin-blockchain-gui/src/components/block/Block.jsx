...
export default function Block() {
  ...
  const poolReward = mojo_to_sesucoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_sesucoin(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const sesucoinFees = blockRecord.fees
    ? mojo_to_sesucoin(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: sesucoinFees ? `${sesucoinFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Sesucoin blockchain
            </Trans>
          </Back>
        }
        action={
