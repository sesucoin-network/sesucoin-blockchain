...
import sesucoinEnvironment from '../util/sesucoinEnvironment';
import sesucoinConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!sesucoinEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    sesucoinConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
     ...
     // console.log('startUrl', startUrl);
      ...
      if (!sesucoinConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !sesucoinConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (sesucoinConfig.manageDaemonLifetime()) {
        sesucoinEnvironment.startSesucoinDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
		  {
            label: i18n._(/* i18n */ { id: 'Sesucoin Blockchain Website' }),
            click: () => {
              openExternal(
                'https://sesucoin.eu',
              );
          ...
          },
		  {
            ...
            click: () => {
              openExternal(
                'https://www.sesucoin.eu/faq',
              );
            },
          },
		  {
            label: i18n._(/* i18n */ { id: 'Sesucoin Calculator' }),
            click: () => {
              openExternal(
          ...
          {
            label: i18n._(/* i18n */ { id: 'Sesucoin Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/sesucoin-network/sesucoin-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/sesucoin-network/sesucoin-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/sesucoin-network/sesucoin-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/sesucoin-network/sesucoin-blockchain/issues',
              );
            },
          },
		  {
		  ...
		  {
            label: i18n._(/* i18n */ { id: 'Join Sesucoin on Discord' }),
            click: () => {
              openExternal('https://discord.gg/wVAd75mJYR');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow Sesucoin on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/sesucoin-networkNet');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Sesucoin Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Sesucoin' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Sesucoin Blockchain' }),
            click: () => {
              openAbout();
            },
          },
