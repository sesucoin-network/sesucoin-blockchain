...
global.daemon_rpc_ws = `wss://${self_hostname}:40040`;
...
function loadConfig(version) {
  try {
    ...
    // 1) SESUCOIN_ROOT environment variable
    // 2) version passed in and determined by the `sesucoin version` call

    // check if SESUCOIN_ROOT is set. it overrides everything else
    console.log("version", version)
    const config_root_dir =
      'SESUCOIN_ROOT' in process.env
        ? process.env.SESUCOIN_ROOT
        : path.join(os.homedir(), '.sesucoin', version);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 40040); // jshint ignore:line
