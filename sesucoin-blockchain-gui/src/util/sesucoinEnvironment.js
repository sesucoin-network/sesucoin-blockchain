...
const PY_FOLDER = '../sesucoin/daemon';
...
const getSesucoinVersion = () => {
  let version = null;
  const exePath = getExecutablePath('sesucoin');
  console.log("exePath", exePath)
  // first see if we can get a sesucoin exe in a standard location relative to where we are
  try {
    ...
    console.log("e1", e1)
    // that didn't work, let's try as if we're in the venv or sesucoin is on the path
    try {
      version = child_process
