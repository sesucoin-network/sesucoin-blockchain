...
from sesucoin.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from sesucoin.protocols import full_node_protocol
from sesucoin.protocols.full_node_protocol import RespondBlock
...
from sesucoin.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
class TestPoolWalletRpc:
    ...
    async def test_absorb_self(self, one_wallet_node_and_rpc, fee):
        ...
        tr: TransactionRecord = await client.send_transaction(
            1, 100, encode_puzzle_hash(status.p2_singleton_puzzle_hash, "tsesucoin")
        )
        await time_out_assert(
            10,
            full_node_api.full_node.mempool_manager.get_spendbundle,
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc, fee):
        ...
        # Balance stars at 6 SESUCOIN and 5 more blocks are farmed, total 22 SESUCOIN
    ...
    async def test_leave_pool(self, setup, fee, target_puzzle_hash):
        ...
        try:
            ...
            async def have_sesucoin():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_sesucoin)
