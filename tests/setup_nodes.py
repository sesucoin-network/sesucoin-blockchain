...
from sesucoin.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from sesucoin.full_node.full_node_api import FullNodeAPI
from sesucoin.server.start_farmer import service_kwargs_for_farmer
from sesucoin.server.start_full_node import service_kwargs_for_full_node
from sesucoin.server.start_harvester import service_kwargs_for_harvester
from sesucoin.server.start_introducer import service_kwargs_for_introducer
from sesucoin.server.start_service import Service
from sesucoin.server.start_timelord import service_kwargs_for_timelord
from sesucoin.server.start_wallet import service_kwargs_for_wallet
from sesucoin.simulator.start_simulator import service_kwargs_for_full_node_simulator
from sesucoin.timelord.timelord_launcher import kill_processes, spawn_process
...
from sesucoin.util.ints import uint16, uint32
from sesucoin.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["xss_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "sesucoin")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xss_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "sesucoin")
