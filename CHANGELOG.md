...
- Beta builds are built on every merge to main, and are now available from <https://sesucoin.eu/download/>.
...
- Switched IP address lookup to first use sesucoin's service ip.sesucoin.eu.
...
- This is a hard fork/breaking change from RC6/7. TSESUCOIN Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.sesucoin.eu/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- We have moved to a new and much higher scalability download.sesucoin.eu to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.sesucoin.eu/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.sesucoin.eu/latest/Setup-MacOS.dmg).
- sesucoin wheels not on Pypi and some dependecies not found there also are now on pypi.sesucoin.eu.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.sesucoin.eu/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [sesucoin.eu](https://sesucoin.eu/). Among the improvements this gives the sesucoin blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- More details on the release at [https://www.sesucoin.eu/developer/](https://www.sesucoin.eu/developer/)
