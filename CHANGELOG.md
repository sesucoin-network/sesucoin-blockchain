...
## 1.2.11 sesucoin blockchain 2021-11-4

Farmers rejoice: today's release integrates two plotters in broad use in the sesucoin community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Alternative--Plotters).
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/sesucoin-network/sesucoin-blockchain/pull/9050).
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the sesucoin Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `sesucoin init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `sesucoin plotnft join`) now accept a fee parameter, but it is not yet operable.

## 1.2.10 sesucoin blockchain 2021-10-25

We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Passphrase-Protected-sesucoin-Keys-and-Key-Storage-Migration). We also launched a new Chialisp compiler in clvm_tools_rs which substantially improves compile time for Chialisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Passphrase-Protected-sesucoin-Keys-and-Key-Storage-Migration).
- Enabled experimental use of a new Chialisp compiler in clvm_tools_rs in sesucoin-blockchain, which is off by default, and substantially improves compile time.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other sesucoin services.
...
- Updated the old comment on sesucoin/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the sesucoin DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 sesucoin blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 sesucoin blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward sesucoin's broader set of DID capabilities.
...
- Beta builds are built on every merge to main, and are now available from <https://sesu.ddns.net/download/>.
...
- We have made many full node changes to support our upcoming sesucoin Asset Token (CAT) standard and our upcoming standalone light wallet, which will use sesucoin's new electrum-style protocol to enable faster wallet syncing.
...
- Several clvm_rs updates to support our upcoming sesucoin Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install sesucoin-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the sesucoin install and then converts it into proper npm format and puts that into package.json.
...
- Thanks @altendky for changing the default to paginate to sesucoin wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use sesucoin's service ip.sesu.ddns.net.
...
## 1.2.3 sesucoin blockchain 2021-07-26
...
- Thanks @mishan for your change to 'sesucoin plotnft show' to display Percent Successful Points.
...
## 1.2.2 sesucoin blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/sesucoin-network/sesucoin-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 sesucoin blockchain 2021-07-12
...
## 1.2.0 sesucoin blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `sesucoin plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/sesucoin-network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `sesucoin farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `sesucoin configure` now supports command line updates to peer count and target peer count.
...
- All sesucoin repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `sesucoin plotnft` to manage pools.
...
- Correctly display private key in `sesucoin keys show`.
- Thanks to @sitecurtins for removing a default printout of the private key mnemonic in `sesucoin keys show`.
...
- Thank you to @sesucoinMineJP for various improvements.
...
- `sesucoin farm summary` better handles wallet errors.
...
### 1.1.7 sesucoin Blockchain 2021-06-05

## 1.1.6 sesucoin Blockchain 2021-06-05
...
## 1.1.6 sesucoin Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /sesucoin/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `sesucoin configure -t true|false`).
...
## 1.1.5 sesucoin Blockchain 2021-05-09
...
## 1.1.4 sesucoin Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `sesucoin keys show`. Use `sesucoin keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 sesucoin Blockchain 2021-05-01
...
## 1.1.2 sesucoin Blockchain 2021-04-24
...
## 1.1.1 sesucoin Blockchain 2021-04-21
...
## 1.1.0 sesucoin Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the sesucoin Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
...
## 1.0.5 sesucoin Blockchain 2021-04-14
...
## 1.0.4 sesucoin Blockchain 2021-04-12
...
- The `src` directory in sesucoin-blockchain has been changed to `sesucoin` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `sesucoin/`
...
## 1.0.3 sesucoin Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/sesucoin-network/sesucoin-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `sesucoin keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 sesucoin Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [chiapos](https://github.com/sesucoin-network/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/sesucoin-blockchain).
...
- @jespino changed `sesucoin keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `sesucoin keys show`.
...
## 1.0.1 sesucoin Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `sesucoin keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/sesucoin-blockchain/).
...
- Users can now pip install e.g. sesucoin-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of sesucoin Blockchain 2021-03-17

### Added

- This is the first production release of the sesucoin Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable SESUCOIN. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/sesucoin-blockchain).
...
- Migration with SESUCOIN_ROOT set does not crash sesucoin init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TSESUCOIN Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.sesu.ddns.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `sesucoin farm summary` will now display TSESUCOIN or SESUCOIN as appropriate.
...
- `sesucoin farm summary` displayed some incorrect amounts.
...
- `sesucoin netspace` now defaults to 1000 blocks to mirror the GUI.
...
- Remove `sesucoin plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `sesucoin keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- Generators have been re-worked in CLVM. We added a chialisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during sesucoinLisp native deserialization.
...
- We have moved to a new and much higher scalability download.sesu.ddns.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.sesu.ddns.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.sesu.ddns.net/latest/Setup-MacOS.dmg).
- sesucoin wheels not on Pypi and some dependecies not found there also are now on pypi.sesu.ddns.net.
...
- PRs and commits to the sesucoin-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TSESUCOIN farmed.
- `sesucoin farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `sesucoin wallet` had various bugs.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `xss_target_address` sections of config.yaml.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of TSESUCOIN will not come forward.
- We now support a "green flag" chain launch process. A new version of the software will poll download.sesu.ddns.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `sesucoin farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/sesucoin-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `sesucoin wallet` now takes get_address to get a new wallet receive address from the CLI.
- `sesucoin plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of sesucoin-blockchain, etc., instead of in blspy.
...
- chiabip158 was set to a sesucoin 1.0 version.
- Comments to Chialisp and clvm source have been updated for all of the Chialisp changes over the proceeding three weeks.
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in sesucoin-blockchain.
...
- Adding Click changed the behavior of `sesucoin keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
...
- All of the sesucoin sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all sesucoin dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `sesucoin -upnp f` was added to disable uPnP.
- `sesucoin plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major chialisp changes. TSESUCOIN from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/sesucoin-blockchain).
- You can obtain a new wallet receive address on the command line with `sesucoin wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/sesucoin-network/sesucoin-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `sesucoin -h` and `sesucoin SUBCOMMAND -h` can be your guide.
...
- `sesucoin plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `sesucoin netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `sesucoin init` only migrates release candidate directories. The versioned sub directories under `~/sesucoin` will be going away before mainnet.
...
- `sesucoin configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/sesucoin-network/sesucoin-blockchain/releases/tag/1.0rc1) was well.
...
- Installers will now be of the pattern sesucoinSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'sesucoin wallet get_transactions' in the command line to see your transactions.
- 'sesucoin wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [sesucoin Blockchain GUI](https://crowdin.com/project/sesucoin-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/sesucoin-network/sesucoin-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `sesucoin plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'sesucoin wallet send' now takes in TSESUCOIN or SESUCOIN as units instead of mojos.
...
- The Beta 27 chain is a hard fork. All TSESUCOIN from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- You can now send and receive transactions with the command line. Try `sesucoin wallet -h` to learn more. Also, `sesucoin wallet` now requires a third argument of `show`, therefor you will use `sesucoin wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [sesucoin blockchain gui](https://crowdin.com/project/sesucoin-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `sesucoin init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `sesucoin init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 TSESUCOIN/SESUCOIN per block for a total of 9216 TSESUCOIN per day from 4608 challenges per day.
- Smart transactions now use an announcement instead of 'coin consumed' or lock methods.
- The GUI is now in a separate submodule repository from sesucoin-blockchain, [sesucoin-blockchain-gui](https://github.com/sesucoin-network/sesucoin-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `sesucoin-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `sesucoin wallet`, was incorrectly displaying TSESUCOIN balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/sesucoin-network/sesucoin-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `sesucoin plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and sesucoinLisp code matches the CI compiled code.
- We have removed the '-r' flag from `sesucoin` as it was being overridden in most cases by the `-r` for restart flag to `sesucoin start`. Use `sesucoin --root-path` instead.
- `sesucoin -h` now recommends `sesucoin netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `sesucoin show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `sesucoin configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `sesucoin plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `sesucoin plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `sesucoin show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `sesucoin show` handles sub block 0 better.
...
- `sesucoin plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `sesucoin plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `sesucoin show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `sesucoin configure --enable-upnp`.
...
- `sesucoin configure` now lets you enable or disable uPnP.
...
- `sesucoin netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `sesucoin show` of the GUI use `sesucoin netspace -d 1000` as `sesucoin netspace` defaults to `-d 192` which is one hour.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 TSESUCOIN. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 TSESUCOIN compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 TSESUCOIN.
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [sesu.ddns.net](https://sesu.ddns.net/). Among the improvements this gives the sesucoin blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
- New consensus means this is a very hard fork. All of your TSESUCOIN from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 TSESUCOIN to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 TSESUCOIN to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- You can now specify which private key to use for `sesucoin plots create`. After obtaining the fingerprint from `sesucoin keys show`, try `sesucoin plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- There is a new cli command - `sesucoin configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `sesucoin configure -h`. Note that if sesucoin services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much TSESUCOIN you've won so far. Harvester and Full Node connections have moved to Advanced Options.
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the sesucoin application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.sesucoin/VERSION/config/`
...
- `sesucoin show -s` has been refactored to support the new consensus.
- `sesucoin netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.sesucoin/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run sesucoin-blockchain.
...
- `sesucoin show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/sesucoin-network/chiapos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of sesucoin dependencies like chiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/sesucoin-network/sesucoin-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install sesucoin-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/sesucoin-network/sesucoin-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/sesucoin-network/sesucoin-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The sesucoin GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/sesucoin-network/sesucoin-blockchain/tree/dev/electron-react/src/locales) of the sesucoin-blockchain repository.
- `sesucoin check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/sesucoin-network/chiapos/releases/tag/0.12.32).
...
- The rpc interfaces of all sesucoin services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test sesucoin since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve sesucoin-blocksesucoinn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from sesucoin Network at backup.chia.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the sesucoin backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.chia.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test sesucoin are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.sesucoin` directory.
...
- `sesucoin init` now automatically discovers previous releases in each new release.

### Fixed

- `sesucoin show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `sesucoin show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now sesucoin Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The sesucoin UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (sesucoin run farmer-only, wallet-only, timelord-only)
...
- `sesucoin show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `sesucoin show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the sesucoin simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the sesucoin implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test sesucoin between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test sesucoin from the earlier two releases however.
...
- `sesucoin show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching sesucoin)
...
- See wallet balances in command line: `sesucoin show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `sesucoin keys sign` and `sesucoin keys verify` to allow farmers to certify their ownership of keys.
...
- `sesucoin start wallet-server` changed to `sesucoin start wallet`, for consistency.
...
- Removed legacy scripts such as sesucoin-stop-server, sesucoin-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/sesucoin-network/bls-signatures) to use the
...
- The sesucoin executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./sesucoin -h` from
`~\AppData\Local\sesucoin-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/sesucoin.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.

### Changed

- Minor changes have been made across the repositories to better support
compiling on OpenBSD. HT @n1000.
- Changed SESUCOIN units to TSESUCOIN units for testnet.
...
"sesucoin plots create" and "sesucoin plots check".
- Add plot directories to config.yaml from the cli with "sesucoin plots add".
...
- The Windows installer is now signed by a sesucoin Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.sesucoin/test-plots/`
- We have completely refactored all networking code towards making each sesucoin service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make sesucoin start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the sesucoin Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting sesucoin now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'sesucoin keys' command line.
...
- We added a `sesucoin keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the sesucoin-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `sesucoin netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `sesucoin-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `sesucoin-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `sesucoin-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `sesucoin-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `sesucoin init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `sesucoin start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `sesucoin-start-harvester`.
...
- The user interface now refers to sesucoin coins with their correct currency code of SESUCOIN.
...
- There was a regression in sesucoin Proof of Space ([chiapos](https://github.com/sesucoin-network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `sesucoin init` then sync on first start will happen automatically. "\$SESUCOIN_ROOT" users will need to delete `$SESUCOIN_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `sesucoin netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during sesucoin init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `sesucoin show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `sesucoin show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/sesucoin-network/sesucoin-blockchain/wiki) and placed in [INSTALL](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of sesucoin action commands. You can run `sesucoin version` or `sesucoin start node` for example. Just running `sesucoin` will show you more options. However `sesucoin-create-plots` continues to use the hyphenated form. Also it's now `sesucoin generate keys` as another example.
- sesucoin start commands like `sesucoin start farmer` and `sesucoin stop node` now keep track of process IDs in a run/ directory in your configuration directory. `sesucoin stop` is unlikely to work on Windows native for now. If `sesucoin start -r node` doesn't work you can force the run/ directory to be reset with `sesucoin start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `sesucoin start wallet-gui` has been chaned to `sesucoin start wallet` which launches but the GUI and server on MacOS and Linux. `sesucoin start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `sesucoin stop all` command sometimes fails, use `sesucoin-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `sesucoin init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `sesucoin-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/sesucoin-network/sesucoin-blockchain/wiki/Updating-beta-software). If you've set `$SESUCOIN_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `sesucoin show`.

### Changed

- `sesucoin-restart-harvester` has been renamed from `sesucoin-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.sesucoin/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the sesucoin blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/sesucoin-network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the sesucoin management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `sesucoin-start-farmer &`. The [README.md](https://github.com/sesucoin-network/sesucoin-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.sesucoin/release/config, plots/ db/, wallet/ etc. This can be overridden by `export SESUCOIN_ROOT=~/.sesucoin` for example which would then put the plots directory in `HomeDirectory/.sesucoin/plots`.
- The libraries sesucoin-pos, sesucoin-fast-vdf, and sesucoin-bip-158 have been moved to their own repositories: [chiapos](https://github.com/sesucoin-network/chiapos), [chiavdf](https://github.com/sesucoin-network/chiavdf), and [chaibip158](https://github.com/sesucoin-network/chiabip158). They are brought in by sesucoin-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/sesucoin-network/bls-signatures).
...
- `sesucoin-regenerate-keys` has been renamed `sesucoin-generate-keys`.
...
- sesucoin fullnode, farmer and harvester now default to logging to sesucoin.log in the sesucoin-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the sesucoin blockchain.
...
- Added color logging
- Now sesucoin_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to sesucoin_full node, sesucoin_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new sesucoin services were started.
...
- This is the first release of the sesucoin testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.sesu.ddns.net/developer/](https://www.sesu.ddns.net/developer/)

[unreleased]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/sesucoin-network/sesucoin-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/sesucoin-network/sesucoin-blockchain/releases/tag/Alpha-1.0
