from sesucoin.protocols import farmer_protocol, full_node_protocol, introducer_protocol, wallet_protocol
...
from sesucoin.util.api_decorators import api_request, execute_task, peer_required
...
class WalletNodeAPI:
    ...
    @peer_required
    @api_request
    async def respond_removals(self, response: wallet_protocol.RespondRemovals, peer: WSsesucoinConnection):
        pass

    async def reject_removals_request(self, response: wallet_protocol.RejectRemovalsRequest, peer: WSsesucoinConnection):
        """
    ...
    @peer_required
    @api_request
    async def respond_additions(self, response: wallet_protocol.RespondAdditions, peer: WSsesucoinConnection):
        pass
    ...
    @peer_required
    @api_request
    async def transaction_ack(self, ack: wallet_protocol.TransactionAck, peer: WSsesucoinConnection):
        """
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSsesucoinConnection
    ):
    ...
    @peer_required
    @api_request
    async def respond_peers(self, request: full_node_protocol.RespondPeers, peer: WSsesucoinConnection):
        if not self.wallet_node.has_full_node():
