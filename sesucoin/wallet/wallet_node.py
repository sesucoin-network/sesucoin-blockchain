...
from sesucoin.daemon.keychain_proxy import (
    KeychainProxy,
...
from sesucoin.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from sesucoin.protocols import farmer_protocol, wallet_protocol
from sesucoin.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from sesucoin.util.byte_types import hexstr_to_bytes
from sesucoin.util.check_fork_next_block import check_fork_next_block
from sesucoin.util.errors import Err, ValidationError
from sesucoin.util.ints import uint32, uint64, uint128
from sesucoin.util.keychain import Keychain
from sesucoin.util.lru_cache import LRUCache
from sesucoin.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from sesucoin.util.path import mkdir, path_from_root
from sesucoin.util.profiler import profile_task
from sesucoin.wallet.block_record import HeaderBlockRecord
from sesucoin.wallet.derivation_record import DerivationRecord
from sesucoin.wallet.settings.settings_objects import BackupInitialized
from sesucoin.wallet.transaction_record import TransactionRecord
from sesucoin.wallet.util.backup_utils import open_backup_file
from sesucoin.wallet.util.wallet_types import WalletType
from sesucoin.wallet.wallet_action import WalletAction
from sesucoin.wallet.wallet_blockchain import ReceiveBlockResult
from sesucoin.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[sesucoinServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'sesucoin keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSsesucoinConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def update_stakings(self, peer: WSsesucoinConnection, height: uint64, farmer_public_key: G1Element) -> None:
        "fetch staking"
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSsesucoinConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSsesucoinConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSsesucoinConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSsesucoinConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSsesucoinConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSsesucoinConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
