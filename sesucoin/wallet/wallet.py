...
from sesucoin.consensus.cost_calculator import calculate_cost_of_program, NPCResult
from sesucoin.full_node.bundle_tools import simple_solution_generator
...
from sesucoin.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from sesucoin.wallet.puzzles.puzzle_utils import (
...
from sesucoin.wallet.secret_key_store import SecretKeyStore
from sesucoin.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_sesucoin(self, sesucoin_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if sesucoin_amount < 0:
            utxos = await self.select_coins(abs(sesucoin_amount), exclude)
        else:
        ...
        sesucoin_amount = spend_value + sesucoin_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": sesucoin_amount}]
