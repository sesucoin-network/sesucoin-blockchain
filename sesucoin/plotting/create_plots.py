...
from sesucoin.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from sesucoin.plotting.util import add_plot_directory, stream_plot_info_ph, stream_plot_info_pk
...
from sesucoin.util.config import config_path_for_filename, load_config
from sesucoin.util.keychain import Keychain
from sesucoin.util.path import mkdir
from sesucoin.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
class PlotKeysResolver:
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'sesucoin keys add', 'sesucoin keys generate' or provide a public key with -f"
            )
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'sesucoin keys add', 'sesucoin keys generate' or provide a public key with -p"
            )
