...
from sesucoin.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from sesucoin.plotters.chiapos import get_chiapos_install_info, plot_sesucoin
from sesucoin.plotters.madmax import get_madmax_install_info, plot_madmax
from sesucoin.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in SESUCOIN_ROOT.
    SESUCOIN_ROOT_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within SESUCOIN_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, sesucoin_plotter, "chiapos", "Chiapos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_sesucoin(args, SESUCOIN_ROOT_path)
    if args.plotter == "madmax":
        plot_madmax(args, SESUCOIN_ROOT_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, SESUCOIN_ROOT_path, root_path)
