...
from sesucoin.plotting.create_plots import resolve_plot_keys
from sesucoin.plotters.plotters_util import run_plotter, run_command
...
def get_madmax_executable_path_for_ksize(plotters_root_path: Path, ksize: int = 32) -> Path:
    ...
    madmax_exec: str = "sesucoin_plot"
    if ksize > 32:
        madmax_exec += "_k34"  # Use the sesucoin_plot_k34 executable for k-sizes > 32
    ...
    if is_madmax_supported():
        ...
        run_command(
            [
                "git",
                "clone",
                "https://github.com/sesucoin-network/sesucoin-plotter-madmax.git",
                MADMAX_PLOTTER_DIR,
            ],
...
def plot_madmax(args, SESUCOIN_ROOT_path: Path, plotters_root_path: Path):
