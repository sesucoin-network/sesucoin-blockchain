...
from sesucoin.cmds.init_funcs import check_keys, sesucoin_init
from sesucoin.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from sesucoin.daemon.keychain_server import KeychainServer, keychain_commands
from sesucoin.daemon.windows_signal import kill
from sesucoin.plotters.plotters import get_available_plotters
from sesucoin.plotting.util import add_plot_directory
from sesucoin.server.server import ssl_context_for_root, ssl_context_for_server
from sesucoin.ssl.create_ssl import get_mozilla_ca_crt
from sesucoin.util.sesucoin_logging import initialize_logging
from sesucoin.util.config import load_config
from sesucoin.util.json_util import dict_to_json_str
from sesucoin.util.keychain import (
    Keychain,
...
from sesucoin.util.service_groups import validate_service
from sesucoin.util.setproctitle import setproctitle
from sesucoin.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting sesucoin.")
    quit()
...
service_plotter = "sesucoin plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "sesucoin": "sesucoin",
        "sesucoin": "sesucoin",
        "sesucoin_wallet": "start_wallet",
        "sesucoin_full_node": "start_full_node",
        "sesucoin_harvester": "start_harvester",
        "sesucoin_farmer": "start_farmer",
        "sesucoin_introducer": "start_introducer",
        "sesucoin_timelord": "start_timelord",
        "sesucoin_timelord_launcher": "timelord_launcher",
        "sesucoin_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["sesucoin", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("sesucoin daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered SESUCOIN_ROOT
    os.environ["SESUCOIN_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with SESUCOIN_ROOT: {os.environ['SESUCOIN_ROOT']}")
    ...
    if service_command == "sesucoin_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in sesucoin_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    sesucoin_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("sesucoin_daemon")
