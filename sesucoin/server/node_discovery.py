...
from sesucoin.protocols import full_node_protocol, introducer_protocol
from sesucoin.protocols.protocol_message_types import ProtocolMessageTypes
from sesucoin.server.address_manager import AddressManager, ExtendedPeerInfo
from sesucoin.server.address_manager_store import AddressManagerStore
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: sesucoinServer = server
    ...
    async def on_connect(self, peer: ws.WSsesucoinConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSsesucoinConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSsesucoinConnection):
