...
from sesucoin.consensus.coinbase import create_puzzlehash_for_pk
from sesucoin.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_sesucoin_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from sesucoin.util.bech32m import encode_puzzle_hash
from sesucoin.util.config import (
    create_default_sesucoin_config,
    initial_config_file,
    load_config,
    save_config,
...
from sesucoin.util.ssl_check import (
...
from sesucoin.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from sesucoin.wallet.derive_chives_keys import master_sk_to_chives_pool_sk, master_sk_to_chives_wallet_sk
from sesucoin.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'sesucoin keys generate'")
        return None
    ...
    all_targets = []
    stop_searching_for_farmer = "xss_target_address" not in config["farmer"]
    stop_searching_for_pool = "xss_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xss_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xss_target_address"):
    ...
    if "xss_target_address" not in config["farmer"]:
        print(
            f"Setting the sesucoin destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["xss_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["xss_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['xss_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xss_target_address" not in config["pool"]:
        print(f"Setting the sesucoin destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["xss_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["xss_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xss_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the XSS destination addresses, edit the `xss_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    sesucoin_ca_crt, sesucoin_ca_key = get_sesucoin_ca_crt_key()
    sesucoin_ca_crt_path = ca_dir / "sesucoin_ca.crt"
    sesucoin_ca_key_path = ca_dir / "sesucoin_ca.key"
    write_ssl_cert_and_key(sesucoin_ca_crt_path, sesucoin_ca_crt, sesucoin_ca_key_path, sesucoin_ca_key)
    ...
    sesucoin_ca_crt, sesucoin_ca_key = get_sesucoin_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, sesucoin_ca_crt, sesucoin_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                sesucoin_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return sesucoin_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def sesucoin_version_number() -> Tuple[str, str, str, str]:
...
def sesucoin_minor_release_number():
    res = int(sesucoin_version_number()[2])
    print(f"Install release number: {res}")
    return res


def sesucoin_full_version_str() -> str:
    major, minor, patch, dev = sesucoin_version_number()
...
def sesucoin_init(
    ...
    """
    if os.environ.get("SESUCOIN_ROOT", None) is not None:
        print(
            f"warning, your SESUCOIN_ROOT is set to {os.environ['SESUCOIN_ROOT']}. "
            f"Please unset the environment variable and run sesucoin init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"sesucoin directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if SESUCOIN_ROOT is set, or if user has run sesucoin init twice
        # before a new update.
        if testnet:
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'sesucoin keys show --show-mnemonic-seed'")
