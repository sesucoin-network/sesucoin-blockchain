...
from sesucoin.cmds.wallet_funcs import print_balance, wallet_coin_unit
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["sesucoin"]))
        ...
        try:
            ...
            while time.time() - start < 10:
                ...
                if len(tx.sent_to) > 0:
                    ...
                    print(f"Do sesucoin wallet get_transaction -f {fingerprint} -tx 0x{tx_record.name} to get status")
                    return None
        except Exception as e:
