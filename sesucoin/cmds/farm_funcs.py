...
from sesucoin.cmds.units import units
...
from sesucoin.rpc.wallet_rpc_client import WalletRpcClient
...
from sesucoin.util.ints import uint16, uint64
from sesucoin.util.misc import format_bytes, format_minutes
from sesucoin.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total sesucoin farmed: {amounts['farmed_amount'] / units['sesucoin']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['sesucoin']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['sesucoin']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print(
                "For details on farmed rewards and fees you should run 'sesucoin start wallet' and 'sesucoin wallet show'"
            )
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'sesucoin wallet show'")
    else:
        print("Note: log into your key using 'sesucoin wallet show' to see rewards for each key")
