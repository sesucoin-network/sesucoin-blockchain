...
@wallet_cmd.command("send", short_help="Send xss to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much xss to send, in XSS", type=str, required=True)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in XSS",
    type=str,
    default="0",
    show_default=True,
    required=True,
)
@click.option("-t", "--address", help="Address to send the XSS", type=str, required=True)
@click.option(
...
@wallet_cmd.command("send_from", short_help="Transfer all xss away from a specific puzzle hash")
@click.option(
    "-p",
    "--rpc-port",
    help=(
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-s", "--source", help="Address to send the XSS from", type=str, required=True)
@click.option("-t", "--address", help="Address to send the XSS", type=str, required=True)
@click.option("-n", "--numcoins", help="Number of coins to send, in coins not XSS", type=int, required=True)
def send_from_cmd(
...
@click.option(
    "--launcher-hash",
    help="Set the launcher hash, you should get it from sesucoin wallet",
    type=str,
    default=None,
)
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
