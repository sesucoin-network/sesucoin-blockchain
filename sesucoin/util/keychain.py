...
DEFAULT_USER = f"user-sesucoin-{CURRENT_KEY_VERSION}"  # e.g. user-sesucoin-1.8
DEFAULT_SERVICE = f"sesucoin-{DEFAULT_USER}"  # e.g. sesucoin-user-sesucoin-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting SESUCOIN_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("SESUCOIN_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from sesucoin.cmds.passphrase_funcs import prompt_for_passphrase
