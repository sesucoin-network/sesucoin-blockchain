...
# Gets the version from the result of "sesucoin version"
...
def update_version():
    with open(f"{os.path.dirname(__file__)}/sesucoin-blockchain-gui/package.json") as f:
        data = json.load(f)

    version: str = "0.0"
    output = subprocess.run(["sesucoin", "version"], capture_output=True)
    ...
    with open(f"{os.path.dirname(__file__)}/sesucoin-blockchain-gui/package.json", "w") as w:
